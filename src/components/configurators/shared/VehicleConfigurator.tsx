          ...
          <div className="grid lg:grid-cols-12 gap-8">
            <style>{`
              ...
              @keyframes glow-pulse {
                ...
                50% { box-shadow: 0 0 30px rgba(157, 78, 221, 0.2); }
              }
              .config-left {
            ...
            <div className="lg:col-span-6 space-y-6 config-left">
              {/* Vehicle Summary */}
              <VehicleSummaryCard
                year={year}
                make={make}
              ...
              <div className="config-card rounded-3xl shadow-2xl p-6 space-y-4 border" style={{ background: 'rgba(255, 255, 255, 0.98)', borderColor: 'rgba(74, 20, 140, 0.1)' }}>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {[
                      ...
                      <button
                        ...
                        className={`service-btn ${serviceType === service.id ? 'service-btn-selected' : ''} p-4 rounded-xl border-2 text-left flex items-center gap-3`}
                        style={{
                          borderColor: serviceType === service.id ? 'var(--auto-plum-neon)' : 'var(--border)',
                          background: serviceType === service.id ? 'linear-gradient(135deg, rgba(74, 20, 140, 0.05), rgba(157, 78, 221, 0.05))' : 'transparent'
                        }}
                      >
                        <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: serviceType === service.id ? 'linear-gradient(135deg, var(--auto-plum-deep), var(--auto-plum-neon))' : 'rgba(0, 0, 0, 0.05)' }}>
                          <service.icon className="w-6 h-6" style={{ color: serviceType === service.id ? 'white' : 'var(--muted-foreground)' }} />
                        </div>
                        <span style={{ fontWeight: 600, color: serviceType === service.id ? 'var(--auto-plum-deep)' : 'var(--foreground)' }}>
                          {service.fullName}
                        </span>
                        {serviceType === service.id && <Check className="w-5 h-5 ml-auto" style={{ color: 'var(--auto-plum-neon)' }} />}
                      </button>
                    ))}
                  </div>
                </AccordionSection>
                    ...
                    {vehicleTypeOptions.map((type) => {
                      ...
                      return (
                        <button
                          ...
                          className="vehicle-type-btn p-4 rounded-xl border-2 text-left"
                          style={{
                            borderColor: vehicleType === type.id ? 'var(--auto-plum-neon)' : 'var(--border)',
                            background: vehicleType === type.id ? 'linear-gradient(135deg, rgba(74, 20, 140, 0.05), rgba(157, 78, 221, 0.05))' : 'transparent'
                          }}
                        >
                          <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-3" style={{ background: vehicleType === type.id ? 'linear-gradient(135deg, var(--auto-plum-deep), var(--auto-plum-neon))' : 'var(--muted)' }}>
                            <VehicleIcon className="w-6 h-6" style={{ color: vehicleType === type.id ? 'white' : 'var(--muted-foreground)' }} />
                          </div>
                          <div style={{ fontSize: '14px', fontWeight: 600, color: vehicleType === type.id ? 'var(--auto-plum-deep)' : 'var(--foreground)' }}>
                            {type.label}
                          </div>
                          {vehicleType === type.id && <Check className="w-4 h-4 mt-2" style={{ color: 'var(--auto-plum-neon)' }} />}
                        </button>
                      );
                    })}
                  </div>
                ...
                {serviceType !== 'tint' && serviceType && (
                  ...
                  >
                    <div className="flex gap-3 p-2 rounded-xl" style={{ background: 'var(--muted)' }}>
                      {(serviceType === 'clear-ppf' 
                        ...
                        <button
                          ...
                          className="finish-toggle flex-1 py-3 px-4 rounded-lg"
                          style={{
                            background: ppfFinish === option.value ? 'linear-gradient(135deg, var(--auto-plum-deep), var(--auto-plum-neon))' : 'transparent',
                            color: ppfFinish === option.value ? 'white' : 'var(--foreground)',
                            fontWeight: 600,
                            boxShadow: ppfFinish === option.value ? '0 4px 12px rgba(74, 20, 140, 0.3)' : 'none'
                          }}
                        >
                          {option.label}
                        </button>
            ...
            <div className="lg:col-span-6 config-right">
            ...
            <div className="lg:col-span-12 mt-8 pt-12 border-t" style={{ borderColor: 'rgba(74, 20, 140, 0.1)' }}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {(
                  ...
                  <div 
                    key={idx} 
                    className="feature-card rounded-2xl border p-6 group"
                    style={{ 
                  ...
                  >
                    <div 
                      ...
                      style={{ 
                        background: 'linear-gradient(135deg, rgba(74, 20, 140, 0.1), rgba(157, 78, 221, 0.1))',
                        boxShadow: '0 4px 12px rgba(74, 20, 140, 0.1)'
                      }}
                    >
                      <feature.icon className="w-7 h-7 transition-all duration-300 group-hover:scale-110" style={{ color: 'var(--auto-plum-neon)' }} />
                    </div>
                    <h3 className="mb-2" style={{ fontWeight: 700, color: 'var(--foreground)' }}>{feature.title}</h3>
                    <p style={{ fontSize: '14px', color: 'var(--muted-foreground)', lineHeight: '1.6' }}>{feature.desc}</p>
                  </div>
                ))}
              </div>
            </div>
